...
Transaction.prototype = {
  ...
  prettyComplete(authorization_url, transaction) {
    ...
    if (bodies.length > 0) {
      const el = bodies[0];
      const notchpay_popup = document.getElementById("notchpay_popup");

      if (notchpay_popup) {
        notchpay_popup.remove();
      }
      el.innerHTML +=
        "<iframe id='notchpay_popup' style='z-index: 2147483647; background: rgba(0, 0, 0, 0.75); width: 100vw; height: 100vh; border: 0px none transparent; overflow: hidden; margin: 0px; padding: 0px; -webkit-tap-highlight-color: transparent; position: fixed; left: 0px; top: 0px; right: 0; bottom: 0; transition: opacity 0.3s ease 0s; visibility: visible;' frameborder='0' allowtransparency='true' src='" +
        authorization_url +
        "'></iframe>iv>";
