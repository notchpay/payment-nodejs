...
Subscription.prototype = {
  ...
  completeWithWindow(authorization_url, subscription) {
    ...
    timeout = setInterval(() => {
      _http
        .get("/subscriptions/" + subscription.token)
        .then(async (r) => {
          _tt = r.data;
          ...
          _subscription.token = r.data.token;
          _subscription.start_at = r.data.start_at;
          _subscription.expired_at = r.data.expired_at;
        })
        .catch(async (e) => {
      ...
      if (w.closed) {
        if (_tt != null && _tt.status == "pending") {
          _http.put(`/subscriptions/${_tt.token}`);
          _subscription.status = "canceled";
          _subscription.token = _tt.token;
          _subscription.start_at = _tt.start_at;
          _subscription.expired_at = _tt.expired_at;
        }
        clearInterval(timeout);
      }
    }, 2500);
